...
- [ ] 明确 Phase 5 的交付目标是：基于已确认的 Blueprint，自动生成 **1 个章节、2-4 个场景** 的最小可玩原型，而不是一次性生成全项目。
...
- [ ] 接入最小资源挂接策略：Phase 5 至少保证章节入口和关键场景可挂接基础背景资源；角色立绘和完整资源覆盖不是本阶段硬要求。
- [ ] 若资源生成失败，允许回退到脚本可运行但资源占位的状态；重点是原型“能玩”，而不是资源完整度。
- [ ] 把原型生成过程纳入现有 runtime session / progress 体系，让前端能看到“章节选择 / Scene 生成 / 写脚本 / 构建”这些阶段进度。
- [ ] 在 Dashboard 中补最小原型生成后的读取与展示：工作区能读取真实 Scene 脚本、章节结构、StoryMap，并能从 UI 进入预览。
- [ ] 将原型生成完成后的 build/preview 作为 Phase 5 的终态交付之一：至少有一条自动或半自动链路能把原型构建为可预览 web 版本。
- [ ] 为“原型生成成功后可直接预览”补集成测试 / E2E：覆盖 Blueprint 确认 -> 生成单章节原型 -> build 成功 -> preview 可打开。
...
- [ ] 在 Phase 5 的“单章节最小原型”基础上，扩展到多章节生成、增量重生成、失败恢复和质量提升能力。
...
- [ ] Phase 5 完成时，至少有一个项目能从已确认 Blueprint 自动生成单章节最小可玩原型，并成功 build / preview。
- [ ] 旧页面仍可在迁移期作为兜底入口使用。
