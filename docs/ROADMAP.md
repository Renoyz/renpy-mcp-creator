...
| 历史真实 E2E | 2026-04 真实模型验证到蓝图冻结（mock 构建）；2026-07-19 真实服务全链路（DeepSeek + DashScope + 真实 SDK）：场景/资产/脚本产出达标，修复 label 连字符与 gallery add 语法两个编译阻塞 bug 后真实 web 构建通过（17s）、预览可运行；CJK 字体注入后中文对话在 web 预览中正常渲染 |
